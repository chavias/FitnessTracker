...
class Config:
    ...
    SQLALCHEMY_TRACK_MODIFICATIONS = os.getenv('SQLALCHEMY_TRACK_MODIFICATIONS', False).lower() in ('false', '0', 'f')
    ...
    MAIL_USE_TLS=os.getenv('TLS').lower() in ('true', '1', 't')
    MAIL_USE_SSL=os.getenv('SSL').lower() in ('true', '1', 't')
